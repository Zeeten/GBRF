...
public class ReadLikeAwardPartTwoModel {

	public List search(ReadLikeAwardPartTwoBean bean) throws ApplicationException {
		return search(bean, 0, 0);
	}
	
	public List search(ReadLikeAwardPartTwoBean bean, int pageNo, int pageSize)
			throws ApplicationException {
		StringBuffer sql = new StringBuffer("SELECT * FROM read_like_award_part_ii  WHERE 1=1");
		...
		try {
			...
			while (rs.next()) {
				bean = new ReadLikeAwardPartTwoBean();
